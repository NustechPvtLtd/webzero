...
class Ion_auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', first_name, last_name, email, users.id, password, active, avatar, social_account, last_login, visitor_count, eccommerce, premium_domain, expiration_type, expiration, plan_id, ip_address, no_of_sites, expiry_date')
                ->join('price_plan', 'price_plan.plan_id = users.price_plan_id', 'left')
                ->join('account_upgrade', 'account_upgrade.user_id = users.id','left')
     ...
     * @author Nustech
     * */
    public function authenticate_user($identity, $password)
    {
    ...
    public function by_pass_login($id)
    {
        ...
        $query = $this->db->select('users.id, first_name, last_name, email, password, active, avatar, social_account, last_login, visitor_count, eccommerce, premium_domain, expiration_type, expiration, plan_id, ip_address, no_of_sites, expiry_date')
                ->join('price_plan', 'price_plan.plan_id = users.price_plan_id', 'left')
                ->join('account_upgrade', 'account_upgrade.user_id = users.id','left') 
                ->where('users.id', $id)
                ->or_where('email', $id)
    ...
    public function update($id, array $data)
    {
        ...
        if (isset($data['group_id'])) {
            ...
            $this->db->update('users_groups', $users_group, array('user_id' => $user->id));
        }
    ...
    public function update_last_login($id)
    {
        ...
        $this->db->update($this->tables['users'], array('last_login' => time(), 'ip_address' => $ip), array('id' => $id));
    ...
    {
        $this->db->select("CONCAT(IFNULL(first_name,''), ' ', IFNULL(last_name,'')) AS name", FALSE);
        $this->db->where('id', $id);
