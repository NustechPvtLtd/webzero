...
class Ion_auth {
     ...
     * @author Mathew
     * */
    public function register($username, $password, $email, $additional_data = array(), $group_ids = array(), $plan_ids = array()) //need to test email activation
    {
        $this->ion_auth_model->trigger_events('pre_account_creation');

        $id = $this->ion_auth_model->register($username, $password, $email, $additional_data, $group_ids, $plan_ids);
        if ($id !== FALSE) {
            $message = $this->load->view('login/'.$this->config->item('email_templates', 'ion_auth') . $this->config->item('email_register', 'ion_auth'), array('username'=>$email,'password'=>$password), true);
            $headers = array();
            ...
            $this->email->to($email);
            $this->email->subject("Thank You for registration and your Login Details Are!");
            $this->email->message($message);
    ...
    public function send_account_activation($id)
    {
        ...
        if (!$this->config->item('use_ci_email', 'ion_auth')) {
            ...
            $headers = array();
            $headers['From'] = "{$this->config->item('site_title', 'ion_auth')}<{$this->config->item('email_send_mail', 'ion_auth')}>";
            $headers['To'] = "Dear {$user->first_name}<{$user->email}>";
            $headers['X-Mailer'] = "PHP/" . phpversion();
            $this->email->clear();
            $this->email->from($this->config->item('email_send_mail', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
            $this->email->to($user->email);
